...
Page({
  ...
  requestQuizGrid: function (quizUserId) {
    return new Promise((resolve, reject) => {
      // 如果缓存数据 quizRecords 存在
      if (QuizGrid.get()) {
        // 直接返回操作成功
        resolve()
      } else { // 否则，获取当前用户的 quizRecords
        console.debug(`获取 quizRecords`)
        qcloud.request({
          url: `${configs.weapp}/quiz_records`,
          success: res => {
            console.debug(`获取 quizRecords 成功`)
            // 更新 quizGrid
            this.updateQuizGrid(res.data.data)
            // 操作成功
            resolve()
          },
          fail: err => {
            console.debug(`获取 quizRecords 失败`)
            // 跳转至 home 页面
            wx.navigateBack({
              delta: 1
            })
  ...
  updateQuizGrid: function (resQuizRecords) {
    console.debug(`更新 quizGrid`)
    // 更新 quizGrid
    let quizGrid = []
    for (let i = 0; i < 10; i++) {
      let quizTab = { quizTabName: `${100 * i + 1}-${100 * (i + 1)}` }
      ...
      for (let j = 0; j < 100; j++) {
        const quizId = 100 * i + j + 1
        const resQuizRecord = resQuizRecords.find((item, index, arr) => {
          return item.quizId === quizId
        })
        let quizCard
        if (resQuizRecord) {
          quizCard = resQuizRecord
        } else {
          quizCard = {
            quizId: quizId,
            quizUnlocked: 0,
    ...
    // 更新 quizUser
    const quizUser = this.data.quizUser
    quizUser.currentQuizTabIndex = 0
    quizUser.currentQuizTabName = quizGrid[0].quizTabName
    this.setData({
      quizUser: quizUser
    })
    QuizUser.set(quizUser)
